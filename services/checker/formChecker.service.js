const checkData = require('../../nkcModules/checkData.js');
const { ThrowCommonError } = require('../../nkcModules/error.js');
...
class FormCheckerService {
  ...
  isLengthInRange(str, minLength, maxLength) {
  ...
  // DOI & ORCID
  isValidDOINumber(doi) {
    const reg = /^10\.\d{4,9}\/\S+$/;
    return reg.test(doi || '');
  }
  checkDOINumberFormat(doi) {
  ...
  isValidORCIDNumber(orcid) {
    const reg = /^\d{4}-\d{4}-\d{4}-\d{3}[0-9X]$/;
    return reg.test(orcid || '');
  }
  checkORCIDNumberFormat(orcid) {
...
module.exports = {
  formCheckerService: new FormCheckerService(),
};
