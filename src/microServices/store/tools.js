...
async function getDiskPath(time) {
  ...
  for (const a of storeConfigs.attachment) {
    ...
    time = new Date(time);
    if (time < startingTime || time >= endTime) {
      continue;
    }
    diskPath = a.path;
    break;
...
async function clearMetadata(filePath, outputFilePath) {
  return new Promise((resolve, reject) => {
    ff(filePath)
      .outputOptions(['-map_metadata', '-1', '-c:a', 'copy', '-c:v', 'copy'])
...
async function getFilePathByTime(year, month) {
  ...
  for (const a of attachment) {
    ...
    for (const mediaTypeTopFolder of mediaTypeTopFolders) {
      const { name: mediaTypeTopFolderName, path: mediaTypeTopFolderPath } =
        mediaTypeTopFolder;
      const mediaTypeBottomFolders = await getChildFolder(
        mediaTypeTopFolderPath,
      );
      for (const mediaTypeBottomFolder of mediaTypeBottomFolders) {
        ...
        const dateFolderPath = PATH.resolve(
          mediaTypeBottomFolderPath,
          `./${yearStr}/${monthStr}`,
        );
        try {
          ...
          for (const file of files) {
            ...
            filesPath.push([
              stat.size,
              PATH.join(
                mediaTypeTopFolderName,
                mediaTypeBottomFolderName,
                yearStr,
                monthStr,
                file,
              ),
