  ...
  cacheForums = async () => {
    ...
    for (const forum of forums) {
      ...
      if (!accessible) {
        ...
        if (relation === 'or') {
          if (gradesId.length === 0 && rolesId.length === 0) {
            continue;
          }
    ...
    for (const gradeId in grades) {
      if (!Object.hasOwn(grades, gradeId)) {
        continue;
      }
    ...
    for (const roleAndGrade in rolesAndGrades) {
      if (!Object.hasOwn(rolesAndGrades, roleAndGrade)) {
        continue;
      }
      ...
      rolesAndGrades[roleAndGrade] = rolesAndGrades[roleAndGrade].concat(
        roles[arr[0]],
      );
      rolesAndGrades[roleAndGrade] = rolesAndGrades[roleAndGrade].concat(
        grades[arr[1]],
    ...
    for (const moderator in moderators) {
      if (!Object.hasOwn(moderators, moderator)) {
        continue;
      }
