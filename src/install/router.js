...
router
  .use('/', async (ctx, next) => {
    if (fs.existsSync(path.resolve(__dirname, './install.lock'))) {
      ctx.throw(
        500,
        '网站已经安装过了，若需重新安装请先移除文件./install/install.lock',
      );
    }
    await next();
  })
  .get('/', async (ctx) => {
    const { query } = ctx;
    ctx.body = pug.renderFile(path.resolve('./install/public/index.pug'), {
      data: {
        config: config,
        step: query.step,
        time: Date.now(),
  ...
  .post('/save', async (ctx, next) => {
    ...
    if (!username) {
      ctx.throw(400, '管理员用户名不能为空');
    }
    if (!password) {
      ctx.throw(400, '管理员密码不能为空');
    }
    if (account.password !== account.password2) {
      ctx.throw(400, '两次输入的管理员密码不相同');
    }
    if (checkString.contentLength(username) > 30) {
      ctx.throw(400, '管理员账号用户名不能超过30字节');
    }
    if (!checkString.checkPass(password)) {
      ctx.throw(400, '密码必须包含字母、数字以及符号三者中的至少两者');
    }
    if (password.length < 8) {
      ctx.throw(400, '密码不能小于8位');
    }
    try {
      ...
      servers: [
        {
          ...
          socketTarget: [`http://127.0.0.1:${server.port}`],
          disabled: false,
        },
      ],
    });
        ...
        async (err) => {
          ...
          if (collections.length !== 0) {
            if (drop) {
              ...
              return reject(
                new Error(`数据库 "${databaseName}" 已存在，请更换数据库名。`),
              );
            }
          }
          resolve();
