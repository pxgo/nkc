...
async function transfer(o) {
  ...
  return new Promise((resolve, reject) => {
    alipaySDK
      .exec(`alipay.fund.trans.uni.transfer`, {
        charset: 'UTF-8',
...
async function receipt(o) {
  let { money, id, title = '', notes = '', backParams = '' } = o;
  if (!id) {
    throwErr('支付宝收款ID不能为空');
  }
  if (money > 0) {
  } else {
    throwErr('支付宝金额错误');
...
function createSign(obj) {
  ...
  const str =
    queryString.unescape(queryString.stringify(map)) + alipayConfig.receipt.key;
