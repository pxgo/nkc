...
shopGoodsSchema.statics.extendProductsInfo = async (products, o) => {
  ...
  if (o.productParam) {
    for (const productParam of productId) {
      productParams = await ShopProductsParamsModel.find({
        productId: productParam,
      }).sort({ order: 1 });
      productParams = await ShopProductsParamsModel.extendParamsInfo(
        productParams,
      );
  ...
  return await Promise.all(
    products.map((p) => {
      ...
      if (o.user) {
        product.user = userObj[p.uid];
      }
      if (o.dealInfo) {
        product.dealInfo = dealInfoObj[p.uid];
      }
      if (o.post) {
      ...
      if (o.thread) {
        product.thread = threadObj[p.tid];
      }
      if (o.productParam) {
        product.productParams = productParamObj[p.productId];
      }
      return product;
    }),
  );
...
shopGoodsSchema.methods.ensurePermission = async function () {
  if (this.disabled) {
    throwErr(403, `商品已被屏蔽，暂无法浏览、收藏、添加到购物车和或购买`);
  }
};
/*
  通过id查询商品
...
shopGoodsSchema.methods.onshelf = async function () {
  ...
  if (!threadInfo || productStatus === 'insale') {
    throwErr(400, '商品已上架，请勿重复提交');
  }
  const ThreadModel = mongoose.model('threads');
  ...
  const { tid, pid } = post;
  await ShopGoodsModel.updateMany(
    { productId },
    {
      $set: {
        tid,
  ...
  await attachmentService.savePostCover(post.pid, imgMaster);
};
...
shopGoodsSchema.statics.getHomeGoods = async () => {
  ...
  let goods = await mongoose
    .model('shopGoods')
    .find({ productId: { $in: homeSettings.shopGoodsId } });
...
shopGoodsSchema.statics.computeSellCount = async (productId) => {
  ...
  if (!product) {
    throwErr(500, `商品ID错误，productId: ${productId}`);
  }
  ...
  const productParams = await ShopProductsParamsModel.find({
