...
forumSchema.statics.createForum = async (displayName, type = 'forum') => {
  ...
  await redisService.cacheForums();
...
forumSchema.statics.getUserCategoriesWithForums = async (props) => {
  ...
  if (limitLevel) {
    ...
    for (let categoryForum of forumsTree) {
      if (
        categoryForum.childrenForums &&
        categoryForum.childrenForums.length > 0
      ) {
        for (const forum of categoryForum.childrenForums) {
          if (forum.childrenForums && forum.childrenForums.length > 0) {
            const latestThreads = await ForumModel.getLatestThreadsFromRedis(
              forum.fid,
            );
