...
schema.statics.getPaymentRecord = async (props) => {
  ...
  if (!['native', 'H5', 'jsApi'].includes(apiType))
    throwErr(400, `微信支付apiType error`);
  if (!['H5', 'native', 'jsApi'].includes(apiType))
    throwErr(500, `weChatPay apiType error. apiType: ${apiType}`);
...
schema.statics.setRecordStatusByNotificationInfo = async (body) => {
  ...
  const { out_trade_no, transaction_id, trade_state, trade_state_desc, payer } =
    data;
  const weChatPayRecord = await WechatPayRecordModel.findOne({
    _id: out_trade_no,
  });
  if (!weChatPayRecord) throwErr(400, `微信支付记录「${out_trade_no}」不存在`);
  if (weChatPayRecord.status !== 'waiting')
    throwErr(400, `微信支付记录「${out_trade_no}」状态已过期`);
  let status;
  let note = '';
...
schema.statics.toUpdateRecord = async (type, id) => {
  ...
  if (!['aliPay', 'wechatPay'].includes(type))
    throwErr(500, `payment type error`);
  let model;
  if (type === 'aliPay') {
  ...
  const { status, from } = await model.findOnly(
    { _id: id },
    { status: 1, from: 1 },
  );
