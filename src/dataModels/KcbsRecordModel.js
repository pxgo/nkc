 ...
 * */
kcbsRecordSchema.statics.insertSystemRecord = async (
  type,
  u,
  ctx,
  additionalReward,
  ...
  try {
    await KcbsRecordModel.insertSystemRecordContent(
      type,
      u,
      ctx,
      additionalReward,
 ...
 * 执行科创币的加减
 * */
kcbsRecordSchema.statics.insertSystemRecordContent = async (
  type,
  u,
  ctx,
  additionalReward,
...
kcbsRecordSchema.statics.extendKcbsRecords = async (records) => {
  ...
  const articlesObject = await ArticleModel.getArticlesObjectByArticlesId([
    ...aid,
  ]);
  const articlesUrl = {};
  ...
  for (const a of articles) {
    ...
    if (a.source === 'column') {
      if (!a.sid) {
        ...
        articleUrl = (
          await ArticleModel.getArticleUrlBySource(
            a._id,
            a.source,
            a.sid.split('-')[0],
          )
      ...
      articleUrl = (
        await ArticleModel.getArticleUrlBySource(a._id, a.source, a.sid)
      ).articleUrl;
    }
  ...
  return records.map((r) => {
    ...
    if (r.tid) {
      r.thread = threadsObj[r.tid];
    }
    if (r.fid) {
      r.forum = forumsObj[r.fid];
    }
    if (r.pid) {
...
kcbsRecordSchema.statics.hideSecretInfo = async (records) => {
  for (const record of records) {
    record.c = '';
    if (record.hideDescription) {
      record.description = '「根据相关法律法规和政策，内容不予显示」';
    }
  }
};
...
kcbsRecordSchema.methods.verifyPass = async function () {
  ...
  const { verify: _verify, ordersId, num, to } = this;
  if (_verify) {
    return;
  }
  if (!_verify) {
