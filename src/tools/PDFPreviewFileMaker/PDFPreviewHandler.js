...
const { maxGetPageCount, maxGetPageScale } = configs.pdfPreview;
async function PDFPreviewHandler({
  ...
  if (pageCount === 1) {
    ...
    if (shouldGetPageCount < maxGetPageCount) {
      pages = await newPdf.copyPages(
        pdfDoc,
        Array.from({ length: shouldGetPageCount }, (v, k) => k),
      );
    } else {
      pages = await newPdf.copyPages(
        pdfDoc,
        Array.from({ length: 8 }, (v, k) => k),
      );
    }
  }
  ...
  const scaledSize = jpgImage.scaleToFit(
    pageSize.width / 3,
    pageSize.height / 2,
  );
