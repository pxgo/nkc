...
const filterPasteHtml = require('../../../src/nkcModules/xssFilters/filterPasteHtml');
export default {
  ...
  methods: {
    ...
    initDomId() {
      ...
      return new Promise((resolve) => {
        ...
        self.domId = `editor_id_${Date.now()}_${Math.round(
          Math.random() * 100000,
        )}`;
    ...
    setPasteImageNodeNKCSourceInfoByUploadingOrder(order, rid) {
      const imageNode = this.editor.document.querySelector(
        `img[data-uploading-order="${order}"]`,
      );
      if (!imageNode) return;
    ...
    modifyUploadedResourceStatus(rid, success, order) {
      ...
      const orderMatch = order
        ? `${ridMatch ? ',' : ''}img[data-uploading-order="${order}"]`
        : '';
      if (!ridMatch && !orderMatch) return;
      const images = this.editor.document.querySelectorAll(
        `${ridMatch}${orderMatch}`,
      );
    ...
    catchRemoteImage() {
      ...
      for (let i = 0; i < images.length; i++) {
        ...
        if (TestTag === 'nkcsource') {
          if (
            ...
            (!imageNode.getAttribute('data-id') ||
              !imageNode.getAttribute('data-char'))
          ) {
        ...
        // 本地资源图片
        if (this.isLocalPictureLink(src)) continue;
        // 属性正确的文章图片
        if (
    ...
    downloadRemoteImage(imageNode, url) {
      ...
      return nkcAPI('/download', 'POST', {
        ...
        .then((res) => {
          ...
          this.setImageNodeNKCSourceInfo(imageNode, rid);
          console.log(
            `External image (${url}) downloaded successfully, server is processing.`,
          );
        })
        .catch((err) => {
          this.setImageNodeStatusIsFailed(imageNode);
    ...
    editorPasteBase64ToImageEventHandle(imageNode) {
      ...
      UploadResource({
        ...
        .then((res) => {
          const { rid } = res.r;
          this.setPasteImageNodeNKCSourceInfoByUploadingOrder(
            imageUploadingOrder,
            rid,
          );
        })
    ...
    editorDropImageEventHandle(event) {
      ...
      if (
        ...
        this.dragEventInnerContentMark
      ) {
        // 拖动的是内部内容，不需要做任何处理
        // 不然拖动编辑器内的图片时会重复上传
        return;
    ...
    initEditorPasteImageEvent() {
      this.editor.document.addEventListener(
        'paste',
        this.editorPasteImageEventHandle,
      );
    },
    removeEditorPasteImageEvent() {
      this.editor.document.removeEventListener(
        'paste',
        this.editorPasteImageEventHandle,
      );
    },
    ...
    initEditorDropImageEvent() {
      this.editor.document.body.addEventListener(
        'drop',
        this.editorDropImageEventHandle,
      );
      // this.editor.document.addEventListener('drop', this.editorDropImageEventHandle);
    },
    removeEditorDropImageEvent() {
      this.editor.document.body.removeEventListener(
        'drop',
        this.editorDropImageEventHandle,
      );
    ...
    initEditorBeforePasteEvent() {
      this.editor.addListener(
        'beforepaste',
        this.editorBeforePasteEventHandler,
      );
    },
    ...
    removeEditorAfterPasteEvent() {
      this.editor.removeListener(
        'afterpaste',
        this.editorAfterPasteEventHandler,
      );
    },
    removeScrollEvent() {
    ...
    setSaveInfo(flag) {
      ...
      const editorDom = $(`#${self.domId}`)
        .find('.edui-editor-toolbarbox')
        .eq(0);
            ...
            self.$refs.resourceSelector.open(
              (data) => {
                ...
                for (let i = 0; i < data.length; i++) {
                  ...
                  editor.execCommand(
                    'inserthtml',
                    resourceToHtml(type, source.rid, source.oname),
                  );
                }
              },
              {
          ...
          onclick: function () {
            self.$refs.stickerSelector.open((res) => {
              ...
              if (res.type === 'emoji') {
                editor.execCommand(
                  'inserthtml',
                  resourceToHtml('twemoji', res.data),
                );
              } else if (res.type === 'sticker') {
                editor.execCommand(
                  'inserthtml',
                  resourceToHtml('sticker', res.data.rid),
                );
              }
            });
          },
    ...
    isNotFormatLink(url) {
      ...
      if (fileDomain) {
        return (
          url.indexOf(`${fileDomain}/r/`) === 0 ||
          url.indexOf(`${window.location.origin}/r/`) === 0
        );
      }
    ...
    isLocalPictureLink(url) {
      ...
      if (fileDomain) {
        return (
          url.indexOf(`blob:${window.location.origin}`) === 0 ||
          url.indexOf(`blob:${fileDomain}`) === 0
        );
      }
